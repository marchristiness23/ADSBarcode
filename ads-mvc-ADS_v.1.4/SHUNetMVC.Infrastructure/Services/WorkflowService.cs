...
{
    public class WorkflowService : IUserService
    {
        private readonly HttpContextBase _httpContext;
        private readonly MemoryCache _cache;

        public WorkflowService(HttpContextBase httpContext, MemoryCache cache)
        {
        ...
        private async Task<DoTransactionResponse> DoTransaction()
        {
            //try
            //{
            //    UriBuilder uriBuilder = new UriBuilder(AimanConstant.Uri);
            ...
            //    query["WhereCondition"] = $"EmpAccount='{username}'";
            //    uriBuilder.Query = query.ToString() ?? string.Empty;
            //    var data = await HttpRequester.Http.GetAsJson<HttpResponse<List<MasterUserData>>>(uriBuilder.Uri, (req) => AddHeaders(req, AimanConstant.XAuth));
            //    if (data.IsSuccessful)
            //    {
            //        return data.Value.Object.FirstOrDefault();
            //    }
        ...
        public string GetCurrentUserInfoImpersonate(string account)
        {
